...
| F59 | promtail on CT 104 (added tonight) reads every container's json log **including Loki's own**, so Loki's output is now shipped into Loki | Not an amplifying loop — Loki's info lines are startup and compaction, not per-ingest — but it is waste, and worth excluding | open |
