...
| | |
|---|---|
| Next action | **waiting on Kenny: the v3.5.0 signature, and four manual checks.** kyu 3.4.0 is live on CT 109 since 19:49 (step-12): the guard drill ran against a copy as the kyu user, the merge-import printed nothing — N = 0 confirms step-10's manual merge — the door holds 8, zero 401 since 18:58. **v3.5.0 is published but unsigned**; `scripts/sign-release.sh v3.5.0` from a plain terminal (the panel does not hand minisign its password). When it lands, its deploy is fix-check-1's measurement and the run is **three steps, in this order** (step-13): a `--check` as the kyu user against a COPY of `/appdata/kyu/kyu-config/data/kyu.db`, which is still at schema 4 — it must report the pending migration, exit 0, and leave the copy at user_version 4 with no kyu.pre-v*.db; then the fix-state-1 guard drill; then the live store as weaker confirmation. Report user_version before AND after, not the exit code — the exit code looked fine while fix-22 was happening. Open on Kenny: four manual checks (paperwork ×2, media posters, kyu-e576 chain test). Also open: gap-15 (a check red by design while D56 stands). Waiting on the clock: fix-events-1 reads after 2026-09-21 19:35 UTC (kyu session's); VmSwap of kvm 100 after 2026-09-21 (F185). |
