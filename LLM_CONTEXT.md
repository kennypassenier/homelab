...
- **Recente wijzigingen:**
  ...
  - Client script `register-local-node.sh` is hernoemd naar het compactere `add-ssh.sh` en is 100% idempotent gemaakt. Het past lokale `~/.ssh/config` aliassen intelligent aan (in plaats van blind toe te voegen) en overschrijft foute instellingen zonder andere hosts te breken.
  - Shell scripts (`bootstrap-lxc.sh`, `add-ssh.sh`, `node-sync.sh`) zijn geüpgraded met CLI arguments (`getopts`) en `--help` functionaliteit voor betere automatisering. `bootstrap-lxc.sh` is nu interactief en haalt stacks dynamisch op, met ondersteuning voor een `.env` bestand op de host. Alle host-scripts hanteren nu de overzichtelijke `[actie]-[object]` naamgevingsconventie.
  ...
  - Geavanceerde Watchtower lifecycle pre-checks toegevoegd (zoals `check-streams.sh` voor Jellyfin) die updates annuleren als er streams actief zijn om zo downtime tijdens gebruik te voorkomen. Watchtower is nu ook ingesteld om zichzelf te updaten over alle stacks heen (`containrrr/watchtower:latest`).
