...
| | |
|---|---|
| Next action | **waiting on Kenny: the v3.4.0 signature, and four manual checks.** kyu 3.3.0 is live on CT 109 since 19:22 UTC (step-11): the fix-state-1 guard drill passed both halves, the door holds all 8 clients, zero 401 since. **v3.4.0 is published but unsigned** — `scripts/sign-release.sh v3.4.0` failed with an empty password in the terminal panel, so a plain terminal is the likely route; the release still carries only `kyu` and SHA256SUMS. Once signed: run the guard drill against 3.4.0 **with `--state-dir` on a copy, not the live store** (fix-22: the 3.3.0 drill migrated the live schema 4→5 as root), then deploy; the import must report 0, and any N > 0 names a gap in step-10's manual merge. Open on Kenny: four manual checks (paperwork ×2, media posters, kyu-e576 chain test — `homelab checks answer e576228c ok`). Also open: gap-15 (a check red by design while D56 stands), fix-21 (who wrote /opt/kyu/bin/kyu as root on 2026-09-19 is unknown), fix-22. Waiting on the clock: after 2026-09-21, VmSwap of kvm 100 (F185). |
