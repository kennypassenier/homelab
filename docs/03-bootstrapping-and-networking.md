...
With your local workstation configured, it is time to provision your actual infrastructure. 
...
Once generated, review the `docker-compose.yml` and `.env` files in `stacks/<stack_name>/`. Customize them as needed, then push them to GitHub. Do not commit `.env` files to version control.
...
2. **Provisioning:** It installs Docker, Docker Compose, and `unattended-upgrades` inside the unprivileged LXC.
