...
- **Recente wijzigingen:**
  ...
  - Shell scripts (`bootstrap-lxc.sh`, `register-local-node.sh`, `node-sync.sh`) zijn geüpgraded met CLI arguments (`getopts`) en `--help` functionaliteit voor betere automatisering. `bootstrap-lxc.sh` is nu interactief en haalt stacks dynamisch op, met ondersteuning voor een `.env` bestand op de host. Alle host-scripts hanteren nu de overzichtelijke `[actie]-[object]` naamgevingsconventie.
