...
| | |
|---|---|
| Next action | **waiting on Kenny: four manual checks.** The kyu door is complete — all 8 clients, ha/newsflash/radarr/sonarr adopted 2026-09-20T18:57Z with the tokens they already held, each proven with a harmless GET answering 204 where it took 401 (step-10); zero 401 in the journal since. gap-10 closed: KYU_DATA_DIR is gone from kyu.env, so 3.3.0's guard passes on CT 109. Open on Kenny: the four manual checks (paperwork ×2, media posters, and **kyu-e576**, the chain test — `homelab checks answer e576228c ok` once a test message reaches Home Assistant; Claude deliberately did not send one, it rings his phone). Also open: gap-15, a check that is red by design every night while D56 stands. In the kyu session: whether kyu gains an idempotent merge-import, and the 3.3.0 release go. Waiting on the clock: after 2026-09-21, VmSwap of kvm 100 again (F185 ballooning floor). |
