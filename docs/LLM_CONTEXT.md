...
- **Deployed Stacks:**
  ...
  - `media`: Contains Sonarr, Radarr, Prowlarr, Bazarr, Jellyfin, Seerr, Promtail, and Watchtower. **New layout:** Each app has a directory `/media/$appname` (GitOps: compose, scripts) and `/media/$appname-config` (persistent config/data, bind mount from host).
